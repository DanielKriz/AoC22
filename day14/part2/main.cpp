...
constexpr std::size_t GRID_SIZE = 400;
...
int main([[maybe_unused]] int argc, char *argv[]) {
    ...
    while (std::getline(file, line)) {
        ...
        for (
            ...
            // This makes scope of the problem much smaller
            coords1.first -= 300;
            coords2.first -= 300;
    ...
    while (!blocked) {
        ...
        while (true) {
            if (grid[sandPos.second + 1][sandPos.first] == aoc::tile::empty) {
                ...
                if (grid[sandPos.second][sandPos.first] == aoc::tile::sand)
                    blocked = true;
