...
int main(int argc, char *argv[]) {
    ...
    while (std::getline(file, line)) {
        const auto [count, from, to] = parseCommand(line);
        // NOTE: to is of type tuple_element, which for some reason cannot be captured by lambda
        const auto tmp_to = to;
        std::for_each(
            stacks[from].cbegin() + (stacks[from].size() - count),
            stacks[from].cend(),
            [&stacks, tmp_to](const auto &crate) {
                stacks[tmp_to].push_back(crate);
            }
        );
